(function () {
	...
	angular
	.module('nativeQDAApp')
	.controller('basicTableCtrl', basicTableCtrl);

	/* @ngInject */
	function basicTableCtrl ($routeParams, analysisService, NgTableParams) {
		var vm = this;
		...
		function categoriesTable() {
			...
			.then(function(response) {
				var repeaterID = 0;
				response.data.categories.forEach(function(object) {
		...
		function conceptsTable() {
			...
			.then(function(response) {
				var repeaterID = 0;
				response.data.concepts.forEach(function(object) {
		...
		function entitiesTable() {
			...
			.then(function(response) {
				var repeaterID = 0;
				response.data.entities.forEach(function(object) {
					object.text = object.text || {};	// If no object child, set object to empty (to prevent undefined error)
		...
		function keywordsTable() {
			...
			.then(function(response) {
				var repeaterID = 0;
				response.data.keywords.forEach(function(object) {
					object.text = object.text || {};	// If no object child, set object to empty (to prevent undefined error)
					object.relevance = object.relevance || {};
					object.emotion = object.emotion || {};
					object.sentiment = object.sentiment || {};
		...
		function relationsTable() {
			...
			.then(function(response) {
				var repeaterID = 0;
				response.data.relations.forEach(function(object) {
		...
		function semanticRolesTable() {
			...
			.then(function(response) {
				var repeaterID = 0;
				response.data.semanticRoles.forEach(function(object) {
