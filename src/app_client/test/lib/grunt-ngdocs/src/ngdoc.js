...
Doc.prototype = {
  ...
  html: function() {
    ...
    if (this.author) {
      dom.tag('span', {
          class: 'email' }, function(dom) {
        dom.text('Author: ');
      });
      dom.tag('a', {
          href: 'mailto:' + self.email,
          class: 'email' }, function(dom) {
        dom.text(self.author);
      });
    }
...
function title(doc) {
  ...
  ****** END OG CODE ******/
  ...
  /****** MODIFICATIONS ******/
  /* We are using the ngdoc type 'overview' for modules */
  /* therefore these modifications add the title and module */
  /* heading to the pages */

  else if (match = text.match(MODULE)) {
    return makeTitle(match[1], 'module', 'module', match[1]);
  }
