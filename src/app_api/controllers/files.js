...
module.exports.signUploadS3 = function(req, res) {
	...
	// If the DB is unreachable then a file cannot be uploaded to S3 since its details can't be saved in the DB
...
module.exports.deleteFileS3 = function(req, res) {
	// Checks the state of the connection to the DB is 'connected'
	// If the DB is unreachable then a file cannot be deleted from S3 since the changes can't be saved in the DB
	if(mongoose.connection.readyState === 1) {
...
module.exports.objectAclS3 = function(req, res) {
	// Checks the state of the connection to the DB is 'connected'
	// If the DB is unreachable then a file cannot be modified since the changes can't be saved in the DB
	if(mongoose.connection.readyState === 1) {
		var key = req.body.key;
